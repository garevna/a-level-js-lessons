...
~~~js
let lazyEvaluatedInfiniteList = ( function* recurse ( arg ) {
...
elements [ Symbol.iterator ] = function* () {
    ...
    while ( itemNum < this.length ) {
        yield ( () => {
            ...
            for ( var attr in this [ itemNum ].attrs )
                elem [ attr ] = this [ itemNum ].attrs [ attr ]
            itemNum++
            return elem
        })()
    }
...
~~~js
const points = {
    first:  { val: { x: 400, y: 30  }, nextPoint: "second" },
    forth:  { val: { x: 100, y: 50  }, nextPoint: "fifth" },
    sixth:  { val: { x: 300, y: 120 }, nextPoint: null },
    third:  { val: { x: 200, y: 90  }, nextPoint: "forth" },
    fifth:  { val: { x: 50,  y: 100 }, nextPoint: "sixth" },
    second: { val: { x: 150, y: 150 }, nextPoint: "third" }
}
