...
Поэтому у функции-испольнителя предучсотрены два формальных параметра - колбэки
...
И при этом, если бы мы не объявили функции **`resolve`** и **`reject`** до вызова функции **`executor`**, было бы сгенерировано исключение **ReferenceError**
